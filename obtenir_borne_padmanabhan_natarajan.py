from pulp import LpMinimize, LpProblem, lpSum, LpVariable, value, PULP_CBC_CMD
...
def obtenir_borne_padmanabhan_natarajan(sommets, aretes,
    ...
    # minimiser doit être une valeur booléenne, False pour la borne supérieure et True pour la borne inférieure
    n = len(sommets)
    if minimiser:
        minimiser_probabilites_univariees = [1 - prob for prob in probabilites_univariees]
        minimiser_probabilites_bivariees = [1 - probabilites_univariees[aretes[k][0]]
        ...
        return 1 - obtenir_borne_padmanabhan_natarajan(sommets=sommets, aretes=aretes,
                                                       probabilites_univariees=minimiser_probabilites_univariees,
                                                       probabilites_bivariees=minimiser_probabilites_bivariees,
                                                       nombre_de_defauts=minimiser_nombre_de_defauts, minimiser=False)
    ...
    alpha = LpVariable.dicts("alpha", (i for i in range(n)))
    beta = LpVariable.dicts("beta", (k for k in range(n - 1)))
    Delta = LpVariable.dicts("Delta", (k for k in range(n - 1)), lowBound=0)
    eta = LpVariable.dicts("eta", (k for k in range(n - 1)), lowBound=0)
    chi = LpVariable.dicts("chi", (k for k in range(n - 1)), lowBound=0)
    gamma = LpVariable.dicts("gamma", (k for k in range(n - 1)), lowBound=0)
    tau = LpVariable.dicts("tau", (i for i in range(n)), lowBound=0)
    x = LpVariable.dicts("x", ((i, s, y, t)
                               for i in range(n)
                               for s in range(d[i] + 1)
                               for y in [0, 1]
                               for t in range(n + 1)))  # (1 - y, N[i][s - 1] - (1 - y) + 1)
    z = LpVariable("z")

    # Fonction objectif
    lp += (lambdaa
           + lpSum(alpha[i] * probabilites_univariees[i] for i in range(n))
           + lpSum(beta[k] * probabilites_bivariees[k] for k in range(n - 1)))
    ...
    for i in range(n):
        lp += (lpSum(Delta[k] - eta[k] for k in range(n - 1) if aretes[k][0] == i)
               + lpSum(Delta[k] - gamma[k] for k in range(n - 1) if aretes[k][1] == i)
               + tau[i] + alpha[i] >= 0)
    for k in range(n - 1):
    ...
    for i in range(n):
        if d[i] >= 1:
            ...
            for t in range(1, N_i_1 - 1 + 1):
                ...
                lp += x[i_1, d_i_1, 0, t - 1] - x[i, 1, 1, t] + alpha[i] >= 0
            for t in range(2, N_i_1 + 1):
                lp += x[i_1, d_i_1, 1, t - 1] - x[i, 1, 1, t] + alpha[i] + beta[aretes.index((i, i_1))] >= 0
    for i in range(n):
        if d[i] >= 2:
