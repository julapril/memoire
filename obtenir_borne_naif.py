...
def obtenir_borne_naif(sommets, aretes,
    ...
    # nombre_de_defauts doit être un entier inférieur ou égal au nombre de sommets
    ...
    G = np.array([[(i // 2 ** j) % 2 for j in reversed(range(n))] for i in range(2 ** n)], dtype=bool)
    # Matrice donnant les couples de variables aléatoires liés par une arete et simultanément égaux à 1
    GG = np.array([[G[i][e[0]] & G[i][e[1]] for e in aretes] for i in range(2 ** n)], dtype=bool)
    # Vecteur de "1"
    ...
    obj = chi if minimiser else -chi  # On minimise l'objectif négatif au lieu de maximiser l'objectif
    bnd = [(0, np.inf)] * 2 ** n
    gauche_eq = np.block([[np.transpose(G)], [np.transpose(GG)], [np.transpose(uns)]])
    droite_eq = np.block([[probabilites_univariees], [probabilites_bivariees], [un]])
